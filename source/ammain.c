...
Boolean ShowAllThings;	/* If true, show all objects */
Boolean ShowAllLines;	/* If true, show all lines */
...
static Byte CheatStrings[ch_maxcheats][CHEATLETTERS+1] = { // order should mirror cheat_e. (Also, CHEATLETTERS + 1 to fix warnings of trailing \0)
...
static Byte CheatLetter[CHEATLETTERS+1] = {"ABCUDLRSE"};    // CHEATLETTERS + 1 to fix warnings of trailing \0
		...
		/* Shift the entire string over 1 char */
			{
		    	...
		    	SourcePtr = CurrentCheat;
		   	 	EndPtr = SourcePtr+(CHEATLETTERS-1);
    			do {
	...
	/* Does the string match a cheat sequence? */
			{
				...
				i = 0;
				do {
                    SourcePtr = &CheatStrings[i][0];	/* Main string pointer */
					if (!memcmp((char *)CurrentCheat,(char *)SourcePtr,CHEATLETTERS)) {	/* Match the strings */
...
#if defined(__3DO__)
extern void DrawLine(Word x1,Word y1,Word x2,Word y2,Word color,Word thickness);
#else
...
static void DrawLine(Word x1,Word y1,Word x2,Word y2,Word color,Word /*thickness*/)
{
...
void AM_Drawer(void)
{
	...
	do {
		...
		/* Figure out color */
			...
			DrawThickLine(x1,y1,x2,y2,color);		/* Draw the line */
...
Skip:
...

	{
		...
		DrawThickLine(x1,y1,x2,y2,GREEN);	/* Draw the triangle */
		DrawThickLine(x2,y2,nx3,ny3,GREEN);
		DrawThickLine(nx3,ny3,x1,y1,GREEN);
	}
	...
	if (ShowAllThings) {
		...
		while (mo != &mobjhead) {		/* Wrapped around? */
			if (mo != pmo) {			/* Not the player? */
				...
				DrawThickLine(x1,y1,x2,y2,LILAC);	/* Draw the triangle */
				DrawThickLine(x2,y2,nx3,y2,LILAC);
				DrawThickLine(nx3,y2,x1,y1,LILAC);
			}
